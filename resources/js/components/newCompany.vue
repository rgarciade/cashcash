<template>
	<v-dialog v-model="active" persistent :key="elementReset" max-width="70%">
		<v-card>
			<v-card-title> Creación de Cliente </v-card-title>
			<v-card-text>
				<v-form ref="form" lazy-validation v-model="valid">
					...
					></v-text-field>
					<v-text-field
						v-model="companyData.banck"
						id="c_banck"
						label="banco"
					></v-text-field>
					<v-text-field
						v-model="companyData.cta"
						id="c_cta"
						label="cuenta"
					></v-text-field>
					<v-text-field
						...
						label="email de facturacion y contacto"
					></v-text-field>
					<v-textarea
						solo
						id="c_notas"
			...
			<v-card-actions>
				<v-btn :disabled="!valid" @click="newCompany" color="success"
					>Crear compañia</v-btn
				>
...
export default {
	...
	methods: Object.assign({}, mapActions(['createCompany']), {
		disableModal() {
		...
		async newCompany() {
			if (this.validate()) {
				...
				this.disableModal()
				this.elementReset = !this.elementReset
			}
		},
		validate() {
