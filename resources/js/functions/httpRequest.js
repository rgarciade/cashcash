...
class httpRequest {
	...
	static post(url, data) {
		return new Promise((resolver, reject) => {
			...
			axios
				.post(`${this.baseUrl}/${url}`, data, getHeaders())
	...
	static path(url, data) {
		return new Promise((resolver, reject) => {
			store.commit('charging')
			axios
				.patch(`${this.baseUrl}/${url}`, data, getHeaders())
				.then(function (response) {
					manageGoodResponse(resolver, reject, response)
				})
				.catch(function (error) {
	...
	static get(url) {
		return new Promise((resolver, reject) => {
			store.commit('charging')
			axios
				.get(`${this.baseUrl}/${url}`, getHeaders())
				.then(function (response) {
					manageGoodResponse(resolver, reject, response)
				})
	...
	static delete(url) {
		return new Promise((resolver, reject) => {
			store.commit('charging')
			axios
				.delete(`${this.baseUrl}/${url}`, getHeaders())
				.then(function (response) {
					manageGoodResponse(resolver, reject, response)
				})
				.catch(function (error) {
...
const manageGoodResponse = (resolver, reject, response) => {
	if (
		...
		((!response.data || !response.data.code) && response.status === 200)
	) {
		resolver(response)
		store.commit('charged')
	}
